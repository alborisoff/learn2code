...
k = int(input('Quantity of numbers: '))  # Requesting quantity of numbers

for i in range(k):  # Step-by-step...
    ni = int(input('Number # ' + str(i + 1) + ': '))  # ...inputing numbers...
...
for i in range(k):  # Step-by-step print...
    print 'Prime number No. ' + str(i + 1) + ' for number ' + str(inarray[i]) + ' equals ' + str(outarray[i])  # ...result.
